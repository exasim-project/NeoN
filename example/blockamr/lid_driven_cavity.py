...
def run(N_cells=64, Re=100, cfl=0.25, n_steps=5000, plot_interval=500, max_size=64, plotfile=True):
    ...
    # --- solution dictionary (cf. fvSolution.solvers['p']) ---
    ...
    for step in range(1, n_steps + 1):
        ...
        #   ddt(U) + div(phi, U) - laplacian(nu, U) = 0
        solve(exp.ddt(U) + exp.div(phi, U) - exp.laplacian(nu_func, U), t=t, dt=dt)
        ...
        #   laplacian(dt, p) = div(U*)
        solve(imp.laplacian(dt, p) == exp.div(U), dt=dt, solution=sol_p)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--max-size", type=int, default=64, help="Max block size for AMR (default: 64)"
    )
