...
def tag_vorticity(U, threshold=50.0, tag_field=None):
    ...
    def _tag(lev, tags, time, ngrow):
        ...
        for mfi in blockamr.MFIterator(U.mf[lev]):
            ...
            dvdx = (
                uy[ng + 1 : ng + 1 + vn[0], ng : ng + vn[1], ng : ng + vn[2]]
                - uy[ng - 1 : ng - 1 + vn[0], ng : ng + vn[1], ng : ng + vn[2]]
            ) / (2 * dx[0])
            dudy = (
                ux[ng : ng + vn[0], ng + 1 : ng + 1 + vn[1], ng : ng + vn[2]]
                - ux[ng : ng + vn[0], ng - 1 : ng - 1 + vn[1], ng : ng + vn[2]]
            ) / (2 * dx[1])
...
def run(
    ...
    omega = CellField(
        mesh, ncomp=1, ngrow=0, name="vorticity", fill_patch=FillPatchCellConservative()
    )
    tag_field = CellField(
        mesh, ncomp=1, ngrow=0, name="tagged", fill_patch=FillPatchCellConservative()
    )
    ...
    for lev in range(mesh.n_levels()):
        ...
        if mf is not None:
            ...
            print(
                f"    lev {lev}: {lev_cells:,} cells, {n_boxes} boxes, "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--nz", type=int, default=4, help="Number of cells in z-direction (default: 4)"
    )
    ...
    parser.add_argument(
        "--rho", type=float, default=80.0, help="Shear layer thickness (default: 80)"
    )
    parser.add_argument(
        "--delta", type=float, default=0.05, help="Perturbation amplitude (default: 0.05)"
    )
    parser.add_argument("--max-size", type=int, default=64, help="Max block size (default: 64)")
    parser.add_argument(
        "--blocking-factor", type=int, default=4, help="AMR blocking factor (default: 4)"
    )
    parser.add_argument("--tile-size", type=int, default=8, help="Pallas tile size (default: 8)")
    parser.add_argument(
        "--max-level", type=int, default=0, help="AMR max refinement level (default: 0)"
    )
    parser.add_argument(
        "--tag-threshold", type=float, default=1.0, help="Vorticity tagging threshold"
    )
    parser.add_argument(
        "--no-plot", action="store_true", default=False, help="Skip plotfile output"
    )
    parser.add_argument(
