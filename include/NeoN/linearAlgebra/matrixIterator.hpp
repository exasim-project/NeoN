...
public:
    ...
    // FIXMECollapse annotationCheck failure on line R67Check failure: // FIXMEStatic checks / FIXME
    // checkView details
...
// FIXME add template againCollapse annotationCheck failure on line R73Check failure: // FIXME add
// template againStatic checks / FIXME checkView details template<typename IndexType = localIdx,
// typename MeshType = UnstructuredMesh>
...
public:
    ...
    )
    {
        ...
        std::cout << "cellfacevalues: " << cellFacesValues.size() << std::endl;
        for (size_t celli = 0; celli < mesh.nCells(); celli++)
        {
            if (celli + 1 >= cellFacesSegments.size())
            {
                std::cout << celli + 1 << ">= " << cellFacesSegments.size() << std::endl;
            }
            ...
            for (localIdx i = 0; i < faces; ++i)
            {
                ...
                if (isOwner)
                {
                    if (faceIdx >= ownOffs.size())
                    {
                        std::cout << faceIdx << ">= ownOffs" << ownOffs.size() << std::endl;
                    }
                    matrixColumnIdxV[startIdx + i] = rowStart + ownOffs[faceIdx];
                }
                else
                {
                    if (faceIdx >= neiOffs.size())
                    {
                        std::cout << faceIdx << ">= neiOffs" << neiOffs.size() << std::endl;
                    }
