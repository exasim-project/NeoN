...
// a function in gmgKernels.hpp, which already carries a *Device / *Host
// twin per kernel; these are the third twin, so the correspondence stays
// reviewable side by side:
//
//   gmgGsColorKokkos       <- gmgGsColor              (gmgKernels.hpp)
//   gmgResidRestrictKokkos <- gmgResidRestrict        (gmgKernels.hpp)
//   gmgProlongAddKokkos    <- gmgProlongAdd           (gmgKernels.hpp)
//
...
// bench/gmgVcycleBench.cpp (the bench harness, which also calls them directly for
