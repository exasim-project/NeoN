...
void registerDSL(nb::module_& m)
{
    ...
    imp_m.def(
        "ddt",
        [](ScalarVolField& phi) { return dsl::imp::ddt<scalar>(phi); },
        nb::keep_alive<0, 1>()
    );
    imp_m.def(
        "ddt", [](VectorVolField& phi) { return dsl::imp::ddt<Vec3>(phi); }, nb::keep_alive<0, 1>()
    );
    ...
    imp_m.def(
        "laplacian", &dsl::imp::laplacian<scalar>, nb::keep_alive<0, 1>(), nb::keep_alive<0, 2>()
    );
    imp_m.def(
        "laplacian", &dsl::imp::laplacian<Vec3>, nb::keep_alive<0, 1>(), nb::keep_alive<0, 2>()
    );
    ...
    exp_m.def(
        "div",
        [](const ScalarSurfField& flux) { return dsl::exp::div(flux); },
        nb::keep_alive<0, 1>()
    );
    exp_m.def(
        "laplacian", &dsl::exp::laplacian<scalar>, nb::keep_alive<0, 1>(), nb::keep_alive<0, 2>()
    );
    exp_m.def(
        "laplacian", &dsl::exp::laplacian<Vec3>, nb::keep_alive<0, 1>(), nb::keep_alive<0, 2>()
    );
    exp_m.def("grad", &dsl::exp::grad, nb::keep_alive<0, 1>());
    exp_m.def(
        "source",
        [](ScalarVolField& coeff) { return dsl::exp::source<scalar>(coeff); },
        nb::keep_alive<0, 1>()
    );
    // Vec3 explicit Su source — wraps a reconstructed cell body force as an rhs operator.
    exp_m.def(
        "source",
        [](VectorVolField& coeff) { return dsl::exp::source<Vec3>(coeff); },
        nb::keep_alive<0, 1>()
    );
    exp_m.def(
        "source",
