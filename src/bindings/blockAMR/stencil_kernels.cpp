        ...
        for (int comp = 0; comp < ncomp; ++comp)
        {
            amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k)
            {
                ...
                // x-direction
                {
                    ...
                    amrex::Real phi_l = (fl >= 0.0)
                        ? sm1 + 0.5 * vanleerCorr(sm1 - sm2, dl)
                        : s0 - 0.5 * vanleerCorr(sp1 - s0, dl);
                    amrex::Real dr = sp1 - s0;
                    amrex::Real phi_r = (fr >= 0.0)
                        ? s0 + 0.5 * vanleerCorr(s0 - sm1, dr)
                ...
                amrex::Real s0 = phi(i, j, k, comp);
                amrex::Real lap =
                    (phi(i + 1, j, k, comp) - 2.0 * s0
                     + phi(i - 1, j, k, comp))
                        * idx2
                    + (phi(i, j + 1, k, comp) - 2.0 * s0
                       + phi(i, j - 1, k, comp))
                          * idy2
                    + (phi(i, j, k + 1, comp) - 2.0 * s0
                       + phi(i, j, k - 1, comp))
                          * idz2;
    ...
    for (amrex::MFIter mfi(phi_mf); mfi.isValid(); ++mfi)
    {
        ...
        for (int comp = 0; comp < ncomp; ++comp)
        {
            amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k)
            {
                ...
                amrex::Real lap =
                    (phi(i + 1, j, k, comp) - 2.0 * s0
                     + phi(i - 1, j, k, comp)) * idx2
                    + (phi(i, j + 1, k, comp) - 2.0 * s0
                       + phi(i, j - 1, k, comp)) * idy2
                    + (phi(i, j, k + 1, comp) - 2.0 * s0
                       + phi(i, j, k - 1, comp)) * idz2;
        ...
        for (int comp = 0; comp < ncomp; ++comp)
        {
            amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k)
            {
                ...
                // x-direction: QUICK (3/8 downstream + 6/8 upwind - 1/8 far-upwind)
                {
                    ...
                    amrex::Real phi_l = (fl >= 0.0)
                        ? 0.375 * s0 + 0.75 * sm1 - 0.125 * sm2
                        : 0.375 * sm1 + 0.75 * s0 - 0.125 * sp1;
                    amrex::Real phi_r = (fr >= 0.0)
                        ? 0.375 * sp1 + 0.75 * s0 - 0.125 * sm1
    ...
    const amrex::Geometry& geom)
{
    ...
    for (amrex::MFIter mfi(phi_mf); mfi.isValid(); ++mfi)
    {
        ...
        amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k)
        {
            ...
            out(i, j, k) =
                (fr_x * ((fr_x >= 0) ? s0 : phi(i+1,j,k))
               - fl_x * ((fl_x >= 0) ? phi(i-1,j,k) : s0)) * idx
              + (fr_y * ((fr_y >= 0) ? s0 : phi(i,j+1,k))
               - fl_y * ((fl_y >= 0) ? phi(i,j-1,k) : s0)) * idy
              + (fr_z * ((fr_z >= 0) ? s0 : phi(i,j,k+1))
               - fl_z * ((fl_z >= 0) ? phi(i,j,k-1) : s0)) * idz;
        });
    }
}
    ...
    const amrex::Geometry& geom)
{
    ...
    for (amrex::MFIter mfi(phi_mf); mfi.isValid(); ++mfi)
    {
        ...
        amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k)
        {
            amrex::Real s0 = phi(i, j, k);
            out(i, j, k) =
                (fx(i+1,j,k) * 0.5*(s0 + phi(i+1,j,k))
               - fx(i,j,k) * 0.5*(phi(i-1,j,k) + s0)) * idx
              + (fy(i,j+1,k) * 0.5*(s0 + phi(i,j+1,k))
               - fy(i,j,k) * 0.5*(phi(i,j-1,k) + s0)) * idy
              + (fz(i,j,k+1) * 0.5*(s0 + phi(i,j,k+1))
               - fz(i,j,k) * 0.5*(phi(i,j,k-1) + s0)) * idz;
        });
    }
}
    ...
    for (amrex::MFIter mfi(phi_mf); mfi.isValid(); ++mfi)
    {
        ...
        amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k)
        {
            ...
            // x
            {
                ...
                amrex::Real dl=s0-sm1;
                amrex::Real pl=(fl>=0)?sm1+0.5*vanleerCorr(sm1-sm2,dl):s0-0.5*vanleerCorr(sp1-s0,dl);
                amrex::Real dr=sp1-s0;
                amrex::Real pr=(fr>=0)?s0+0.5*vanleerCorr(s0-sm1,dr):sp1-0.5*vanleerCorr(sp2-sp1,dr);
    ...
    for (amrex::MFIter mfi(phi_mf); mfi.isValid(); ++mfi)
    {
        ...
        amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k)
        {
            ...
            // x
            {
                ...
                amrex::Real pl=(fl>=0)?0.375*s0+0.75*sm1-0.125*sm2:0.375*sm1+0.75*s0-0.125*sp1;
                amrex::Real pr=(fr>=0)?0.375*sp1+0.75*s0-0.125*sm1:0.375*s0+0.75*sp1-0.125*sp2;
                divF+=(fr*pr-fl*pl)*idx;
            }
            // y
            {
        ...
        for (int iz = 0; iz < nz; ++iz) {
            for (int iy = 0; iy < ny; ++iy) {
                for (int ix = 0; ix < nx; ++ix) {
                    ...
                    fx_list.append(fx_global_off + ix + (nx + 1) * iy + (nx + 1) * ny * iz);
                    fy_list.append(fy_global_off + ix + nx * iy + nx * (ny + 1) * iz);
                    fz_list.append(fz_global_off + ix + nx * iy + nx * ny * iz);
                }
            }
        }
...
void
registerStencilKernels(nb::module_& m)
{
    m.def(
    ...
    m.def("div_upwind",
        [](amrex::MultiFab& out, const amrex::MultiFab& phi,
           const amrex::MultiFab& fx, const amrex::MultiFab& fy,
           const amrex::MultiFab& fz, const amrex::Geometry& geom)
        { divUpwind(out, phi, fx, fy, fz, geom); },
        nb::arg("out"), nb::arg("phi"), nb::arg("fx"), nb::arg("fy"),
        nb::arg("fz"), nb::arg("geom"),
    ...
    m.def("div_linear",
        [](amrex::MultiFab& out, const amrex::MultiFab& phi,
           const amrex::MultiFab& fx, const amrex::MultiFab& fy,
           const amrex::MultiFab& fz, const amrex::Geometry& geom)
        { divLinear(out, phi, fx, fy, fz, geom); },
        nb::arg("out"), nb::arg("phi"), nb::arg("fx"), nb::arg("fy"),
        nb::arg("fz"), nb::arg("geom"),
    ...
    m.def("div_vanleer",
        [](amrex::MultiFab& out, const amrex::MultiFab& phi,
           const amrex::MultiFab& fx, const amrex::MultiFab& fy,
           const amrex::MultiFab& fz, const amrex::Geometry& geom)
        { divVanLeer(out, phi, fx, fy, fz, geom); },
        nb::arg("out"), nb::arg("phi"), nb::arg("fx"), nb::arg("fy"),
        nb::arg("fz"), nb::arg("geom"),
    ...
    m.def("div_quick",
        [](amrex::MultiFab& out, const amrex::MultiFab& phi,
           const amrex::MultiFab& fx, const amrex::MultiFab& fy,
           const amrex::MultiFab& fz, const amrex::Geometry& geom)
        { divQuick(out, phi, fx, fy, fz, geom); },
        nb::arg("out"), nb::arg("phi"), nb::arg("fx"), nb::arg("fy"),
        nb::arg("fz"), nb::arg("geom"),
            ...
            for (amrex::MFIter mfi(phi_mf); mfi.isValid(); ++mfi)
            {
                ...
                amrex::ParallelFor(
                    bx, [=] AMREX_GPU_DEVICE(int i, int j, int k)
                    {
                        ...
                        out(i, j, k) =
                            (phi(i + 1, j, k) - 2.0 * c + phi(i - 1, j, k))
                                * idx2
                            + (phi(i, j + 1, k) - 2.0 * c + phi(i, j - 1, k))
                                  * idy2
                            + (phi(i, j, k + 1) - 2.0 * c + phi(i, j, k - 1))
                                  * idz2;
                    });
            }
        },
    ...
    m.def(
        ...
        nb::arg("out"), nb::arg("phi"), nb::arg("geom"), nb::arg("ncomp") = 1,
    ...
    m.def(
            ...
            for (amrex::MFIter mfi(phi_mf); mfi.isValid(); ++mfi)
            {
                ...
                amrex::ParallelFor(
                    ...
                    [=] AMREX_GPU_DEVICE(int i, int j, int k, int n)
                    {
                        ...
                        amrex::Real Fl_x = fl_x * (fl_x >= 0 ? phi(i-1,j,k,n) : phi(i,j,k,n));
                        amrex::Real Fr_x = fr_x * (fr_x >= 0 ? phi(i,j,k,n) : phi(i+1,j,k,n));

                        // y-direction
                        amrex::Real fl_y = fy(i, j, k);
                        amrex::Real fr_y = fy(i, j+1, k);
                        amrex::Real Fl_y = fl_y * (fl_y >= 0 ? phi(i,j-1,k,n) : phi(i,j,k,n));
                        amrex::Real Fr_y = fr_y * (fr_y >= 0 ? phi(i,j,k,n) : phi(i,j+1,k,n));

                        // z-direction
                        amrex::Real fl_z = fz(i, j, k);
                        amrex::Real fr_z = fz(i, j, k+1);
                        amrex::Real Fl_z = fl_z * (fl_z >= 0 ? phi(i,j,k-1,n) : phi(i,j,k,n));
                        amrex::Real Fr_z = fr_z * (fr_z >= 0 ? phi(i,j,k,n) : phi(i,j,k+1,n));
        ...
        nb::arg("out"), nb::arg("phi"),
        nb::arg("fx"), nb::arg("fy"), nb::arg("fz"),
        nb::arg("geom"), nb::arg("ncomp") = 1,
        "Multi-component first-order upwind divergence.");
}
