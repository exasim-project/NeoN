...
static TileLayout
buildTileLayout(MultiFab& mf, int bf)
{
    if (!mf.singleChunkPtr())
        throw std::runtime_error(
            "build_tile_layout: MultiFab not in single-chunk mode");
    ...
    for (MFIter mfi(mf); mfi.isValid(); ++mfi)
    {
        ...
        if (box_offset > (size_t)INT32_MAX)
            throw std::runtime_error(
                "build_tile_layout: buffer offset exceeds int32 range");
        ...
        for (int ti = 0; ti < vNx / bf; ++ti)
        {
            for (int tj = 0; tj < vNy / bf; ++tj)
            {
                for (int tk = 0; tk < vNz / bf; ++tk)
                {
                    ...
                    int cj = ng + tj * bf;
                    int ck = ng + tk * bf;
                    int32_t offset = (int32_t)box_offset
                                   + ci * sx + cj * sy + ck * sz;
...
void
registerTileLayout(nb::module_& m)
{
