...
EllSparsityPattern<IndexType>::EllSparsityPattern(
    Vector<IndexType>&& colIdx, Dimensions dim, localIdx numStoredElementsPerRow, localIdx stride
)
    : dimensions_(dim), colIdxs_(std::move(colIdx)),
      numStoredElementsPerRow_(numStoredElementsPerRow), stride_(stride)
{
    validate();
}
