...
// (persistent.cpp / faceCoeffOp.cpp / mlmgOps.cpp via blockamr_solvers,
// gmgKokkos/apply.cpp and bench/gmgVcycleBench.cpp via blockamr_kokkos) -- the
// exact nvcc trap T2 already hit for the fused Kokkos kernels (see
// gmgKokkos/kernels.cpp). The fix is the same one gmg/gmgKernels.cpp uses:
...
// because apply.cpp and gmgVcycleBench.cpp need these symbols in a build
// WITHOUT Ginkgo, where blockamr_solvers does not exist at all
// (blockAmr/CMakeLists.txt). gmgKernels.hpp is included for GmgFab and
...
// The FP64 operator MultiFab (faceCoeffOp.cpp, persistent.cpp) and the three
// level storage types the GMG hierarchy is built in (persistent.cpp via
// gmgPrecond.hpp, apply.cpp and gmgVcycleBench.cpp via vcycle.hpp).
template void
