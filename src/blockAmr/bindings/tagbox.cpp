                ...
                if (srcOnDevice)
                {
                    ...
                    amrex::ParallelFor(
                        ...
                        [=] AMREX_GPU_DEVICE(int i, int j, int k)
                        {
                            ...
                            if (mdata[idx] != 0) tag4(i, j, k) = TagBox::SET;
                        }
                    );
    ...
    nb::class_<TagBoxArray>(m, "TagBoxArray")
        ...
        .def(
            ...
            [](TagBoxArray& tba, nb::object mfi_obj, nb::ndarray<nb::ro> mask)
            {
                ...
                if (mask.ndim() >= 2) isFortran = (mask.stride(0) <= mask.stride(mask.ndim() - 1));
                ...
                if (srcOnDevice)
                {
                    ...
                    amrex::ParallelFor(
                        ...
                        [=] AMREX_GPU_DEVICE(int i, int j, int k)
                        {
                            ...
                            int lj = j - lo.y;
                            int lk = k - lo.z;
                            Long idx = isFortran ? li + (Long)lj * nx + (Long)lk * nx * ny
                ...
                else
                {
                    ...
                    for (int li = 0; li < nx; ++li)
                        for (int lj = 0; lj < ny; ++lj)
                            ...
                            {
                                Long src_idx = isFortran ? li + (Long)lj * nx + (Long)lk * nx * ny
                                                         : (Long)li * ny * nz + (Long)lj * nz + lk;
                                Long dst_idx = li + (Long)lj * nx + (Long)lk * nx * ny;
                                hostBuf[dst_idx] =
                                    (mdata[src_idx] != 0) ? TagBox::SET : TagBox::CLEAR;
                            }
                    Gpu::htod_memcpy_async(tagBuf, hostBuf, npts * sizeof(char));
                    Gpu::streamSynchronize();
                    const char* src = tagBuf;
