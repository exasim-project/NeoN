...
public:
    ...
    ExternalArena(void* p, std::size_t sz, amrex::Arena* fallback, bool device, bool host)
        : m_ptr(p), m_size(sz), m_fallback(fallback), m_chunk_served(false),
          m_device_accessible(device), m_host_accessible(host)
    {}
...
copyToFab_async(amrex::MultiFab& mf, amrex::MFIter& mfi, nb::ndarray<nb::ro> src)
{
    ...
    if (srcIsFortran)
    {
        ...
        if (fabBox == bx)
        {
            if (dstOnDevice) Gpu::dtod_memcpy_async(dstPtr, devSrc, nbytes);
            else
        ...
        if (dstOnDevice)
        {
            ParallelFor(
                ...
                [=] AMREX_GPU_DEVICE(int i, int j, int k, int n)
                {
                    ...
                    int lj = j - lo[1];
                    int lk = k - lo[2];
                    size_t cIdx = ((size_t)li * ny * nz * ncomp) + ((size_t)lj * nz * ncomp)
                                + ((size_t)lk * ncomp) + n;
...
void registerMultiFab(nb::module_& m)
{
    ...
    nb::class_<BoxList>(m, "BoxList")
        .def(nb::init<>())
        .def(
            "push_back", [](BoxList& bl, const Box& bx) { bl.push_back(bx); }, nb::arg("bx")
        )
    ...
    nb::class_<MultiFab>(m, "MultiFab")
               ...
               nb::ndarray<> data)
            {
                ...
                if (provided != expected)
                {
                    throw std::runtime_error(
                        "Buffer size mismatch: expected " + std::to_string(expected)
                        + " elements, got " + std::to_string(provided)
                    );
                }
        ...
        .def_static(
            ...
            [](const BoxArray& ba, const DistributionMapping& dm, int ncomp, int ngrow)
            {
                IntVect ng(ngrow);
                int64_t total = 0;
                for (int i = 0; i < ba.size(); ++i)
        ...
        .def(
            "set_val", [](MultiFab& mf, double val) { mf.setVal(val); }, nb::arg("val")
        )
        .def_prop_ro(
            "is_device",
            [](const MultiFab& mf)
        ...
        .def(
            ...
            [](nb::object self)
            {
                ...
                for (amrex::MFIter mfi(mf); mfi.isValid(); ++mfi)
                {
                    ...
                    size_t shape[4] = {
                        (size_t)bx.length(0), (size_t)bx.length(1), (size_t)bx.length(2), (size_t)nc
                    };
                    result.append(nb::ndarray<nb::jax, Real, nb::ndim<4>>(
                        fab.dataPtr(), 4, shape, self, nullptr, nb::dtype<Real>(), devType, 0, 'F'
                    ));
                }
                return result;
            }
        ...
        .def(
            ...
            [](nb::object self)
            {
                ...
                if (!ptr) throw std::runtime_error("MultiFab not allocated with single-chunk mode");
                size_t total = mf.singleChunkSize() / sizeof(Real);
                size_t shape[1] = {total};
                int devType = deviceTypeFromArena(mf);
                return nb::ndarray<nb::jax, Real, nb::ndim<1>>(
                    ptr, 1, shape, self, nullptr, nb::dtype<Real>(), devType, 0, 'F'
                );
            }
        )
        .def(
            ...
            [](MultiFab& mf, nb::ndarray<nb::ro> src)
            {
                ...
                if (!dst) throw std::runtime_error("MultiFab not allocated with single-chunk mode");
                size_t nbytes = mf.singleChunkSize();
                size_t src_nbytes = src.size() * src.itemsize();
                if (src_nbytes != nbytes)
                    throw std::runtime_error(
                        "copy_from_flat: size mismatch — src has " + std::to_string(src_nbytes)
                        + " bytes, MultiFab has " + std::to_string(nbytes)
                    );
                const void* src_ptr = src.data();
                bool srcOnDevice = (src.device_type() != nb::device::cpu::value);
                if (srcOnDevice) Gpu::dtod_memcpy(dst, src_ptr, nbytes);
                else
        ...
        .def(
            ...
            [](MultiFab& mf)
            {
                ...
                for (amrex::MFIter mfi(mf); mfi.isValid(); ++mfi)
                {
                    ...
                    int ny = bx.length(1);
                    int nz = bx.length(2);
                    result.append(nb::make_tuple((int64_t)offset, nx, ny, nz, nc));
        ...
        .def(
            ...
            [](MultiFab& mf, int bf, int requested_padded)
            {
                if (!mf.singleChunkPtr())
                    throw std::runtime_error("MultiFab not allocated with single-chunk mode");
                    ...
                    for (int ti = 0; ti < vNx / bf; ++ti)
                    {
                        for (int tj = 0; tj < vNy / bf; ++tj)
                        {
                            for (int tk = 0; tk < vNz / bf; ++tk)
                            {
                                ...
                                int ck = tk * bf;
                                int64_t tile_off =
                                    (int64_t)box_offset + ci * sx + cj * sy + ck * sz;
                ...
                auto make_array = [&](int64_t* host_ptr)
                {
                    ...
                    auto* dev_ptr = static_cast<int64_t*>(The_Device_Arena()->alloc(nbytes));
                    Gpu::htod_memcpy(dev_ptr, host_ptr, nbytes);
                    delete[] host_ptr;
                    auto owner =
                        nb::capsule(dev_ptr, [](void* p) noexcept { The_Device_Arena()->free(p); });
            ...
            [](MultiFab& mf, int bf, int requested_padded)
            {
                ...
                for (amrex::MFIter mfi(mf); mfi.isValid(); ++mfi)
                {
                    ...
                    if (box_offset > (size_t)INT32_MAX)
                        throw std::runtime_error("packed_tiles: buffer offset exceeds int32 range");
                    ...
                    for (int ti = 0; ti < bi.vNx / bf; ++ti)
                    {
                        for (int tj = 0; tj < bi.vNy / bf; ++tj)
                        {
                            for (int tk = 0; tk < bi.vNz / bf; ++tk)
                            {
                                ...
                                int cj = ng + tj * bf;
                                int ck = ng + tk * bf;
                                int32_t offset = (int32_t)bi.offset + ci * sx + cj * sy + ck * sz;
                ...
                auto tiles_arr = nb::ndarray<nb::jax, int32_t, nb::ndim<1>>(
                    dev_ptr, 1, shape, owner, nullptr, nb::dtype<int32_t>(), kDeviceArenaDevice, 0
                );
        ...
        .def(
               ...
               int requested_padded)
            {
                if (!cell_mf.singleChunkPtr())
                    throw std::runtime_error("Cell MultiFab not allocated with single-chunk mode");
        ...
        .def(
            ...
            [](MultiFab& mf, MFIterator& mfi, nb::ndarray<nb::ro> src)
            {
                ...
                bool dstOnDevice =
                    mf.arena()->isDeviceAccessible() && !mf.arena()->isHostAccessible();
                if (dstOnDevice) amrex::Gpu::streamSynchronize();
                if (owns)
                {
        ...
        .def(
            ...
            [](MultiFab& mf, MFIterator& mfi, nb::ndarray<nb::ro> src)
            {
                ...
                bool srcIsFortran = false;
                if (src.ndim() >= 2) srcIsFortran = (src.stride(0) <= src.stride(src.ndim() - 1));
                    ...
                    if (dstOnDevice)
                    {
                        ParallelFor(
                            ...
                            [=] AMREX_GPU_DEVICE(int i, int j, int k, int n)
                            {
                                ...
                                int lj = j - lo[1];
                                int lk = k - lo[2];
                                size_t cIdx = ((size_t)li * ny * nz * nc) + ((size_t)lj * nz * nc)
                                            + ((size_t)lk * nc) + n;
                                arr4(i, j, k, n) = cSrc[cIdx];
                            }
                        );
                                ...
                                for (int j = lo3.y; j <= hi3.y; ++j)
                                    for (int i = lo3.x; i <= hi3.x; ++i)
                                    {
                                        ...
                                        size_t cIdx = ((size_t)li * ny * nz * nc)
                                                    + ((size_t)lj * nz * nc) + ((size_t)lk * nc)
                                                    + n;
                                        arr4(i, j, k, n) = cSrc[cIdx];
                                    }
                    }

                    if (dstOnDevice || srcOnDevice) Gpu::streamSynchronize();
                    if (ownDevSrc)
                    {
                        ...
                        if (dstOnDevice)
                        {
                            ParallelFor(
                                ...
                                [=] AMREX_GPU_DEVICE(int i, int j, int k, int n)
                                {
                                    ...
                                    size_t cIdx = ((size_t)li * ny * nz * nc)
                                                + ((size_t)lj * nz * nc) + ((size_t)lk * nc) + n;
                                    arr4(i, j, k, n) = cSrc[cIdx];
                                }
                            );
                        }
        ...
        .def(
            ...
            [](MultiFab& mf, const Geometry& geom, nb::list bc_types_list, nb::list bc_values_list)
            {
                using namespace amrex;
                ...
                bool onDevice = mf.arena()->isDeviceAccessible() && !mf.arena()->isHostAccessible();
                                ...
                                if (onDevice)
                                {
                                    ...
                                    ParallelFor(
                                        ...
                                        [=] AMREX_GPU_DEVICE(int i, int j, int k, int n)
                                        {
                                            ...
                                            if (bct == 1) arr(i, j, k, n) = 2.0 * bv[n] - interior;
                                            else if (bct == 3)
                                                arr(i, j, k, n) = (n == dir) ? -interior : interior;
                                            else
                                        ...
                                        for (int k = lo.z; k <= hi.z; ++k)
                                            for (int j = lo.y; j <= hi.y; ++j)
                                                for (int i = lo.x; i <= hi.x; ++i)
                                                {
                                                    ...
                                                    Real interior = arr(ic, jc, kc, n);
                                                    if (bct == 1)
                                                        arr(i, j, k, n) = 2.0 * bv[n] - interior;
                                                    else if (bct == 3)
                                                        arr(i, j, k, n) =
                                                            (n == d) ? -interior : interior;
                                                    else
                                                        arr(i, j, k, n) = interior;
                                                }
                                }
                ...
                if (onDevice) Gpu::streamSynchronize();
            },
            nb::arg("geom"),
            nb::arg("bc_types"),
            nb::arg("bc_values")
