...
class Mesh:
    ...
    def __init__(self, ba, dm, geom):
        ...
        self._geom = geom
        self._fields = []
        # ``body`` is set by the caller (e.g. the mesh factory, from meshDict).
        self.body = None
        self._ibm_data = {}
    ...
    # Single-level ``Mesh`` never regrids, so there is no rebuild hook here.

    def build_ibm(self, methods):
        """Eagerly precompute each method's masks/fractions from ``self.body``.

        ``methods`` is a list of IBM strategy CLASSES, e.g. ``[DirectForcing]`` or
        ``IBM.lookup(name)``.
        """
    ...
    def ibm_data(self, method):
        """Data ``build_ibm`` precomputed for ``method``; raises if it was never built."""
...
class AmrMesh:
    ...
    def _rebuild_ibm(self):
        """Recompute per-method IBM data for the current box arrays (regrid hook).

        Masks are spatial and must be rebuilt; ``force_history`` is a time series and
        is carried forward onto the rebuilt data.
        """
    ...
    # AmrCore callbacks — dispatch to the registered fields.
