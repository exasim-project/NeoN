...
def build_local_offsets(bx, by, bz, ng):
    ...
    for ci in range(n_cells):
        ix = ci % bx; it = (ci // bx) % by; iz = ci // (bx * by)
        base[ci] = (ng+ix) + Nx_g*(ng+it) + Nx_g*Ny_g*(ng+iz)
        fx[ci] = ix + (bx+1)*it + (bx+1)*by*iz
        fy[ci] = ix + bx*it + bx*(by+1)*iz
        fz[ci] = ix + bx*it + bx*by*iz
    return LocalOffsets(
    ...
    if mf is not None and fx_mf is not None:
        ...
        for b in range(n_boxes):
            ...
            for ci in range(n_valid):
                ix = ci % bx; it = (ci // bx) % by_; iz = ci // (bx * by_)
                bases.append(cell_off + (ng+ix) + Nx_g*(ng+it) + Nx_g*Ny_g*(ng+iz))
                fx_offs.append(fxo + ix + (bx+1)*it + (bx+1)*by_*iz)
                fy_offs.append(fyo + ix + bx*it + bx*(by_+1)*iz)
                fz_offs.append(fzo + ix + bx*it + bx*by_*iz)
