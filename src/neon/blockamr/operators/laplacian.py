...
class Laplacian:
    ...
    def __rmul__(self, scalar):
        obj = Laplacian.__new__(Laplacian)
    ...
    def build_kernel(self, bucket, t):
        ...
        if isinstance(self.gamma, (int, float)):
            return self.scheme.build_kernel(
                bucket.dh_arr, coeff=self.coeff * self.gamma, ncomp=ncomp,
            )
        ...
        if abs(g1 - g2) < 1e-14 * (abs(g1) + 1.0):
            return self.scheme.build_kernel(
                bucket.dh_arr, coeff=self.coeff * g1, ncomp=ncomp,
            )

        # Variable gamma — evaluate on grown-box grids
        gamma_buf, gamma_offsets = _build_gamma_buffer(
            self.gamma, mf, geom, bucket, t,
        )
        return self.scheme.build_kernel(
            bucket.dh_arr, coeff=self.coeff, ncomp=ncomp,
...
def _build_gamma_buffer(gamma_func, mf, geom, bucket, t):
    ...
    for mfi in blockamr.MFIterator(mf):
        ...
        xs = jnp.array([prob_lo[0] + (lo[0] - ng + i + 0.5) * dx[0]
                        for i in range(bNx)])
        ys = jnp.array([prob_lo[1] + (lo[1] - ng + j + 0.5) * dx[1]
                        for j in range(bNy)])
        zs = jnp.array([prob_lo[2] + (lo[2] - ng + k + 0.5) * dx[2]
                        for k in range(bNz)])
