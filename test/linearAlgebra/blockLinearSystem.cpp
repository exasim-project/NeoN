...
TEST_CASE("BlockLinearSystem")
{
    ...
    SECTION("2x2 source-term system on single cell " + execName)
    {
        //   [4  1] [a]   [11]     →  a = 2, b = 3
        //   [1  3] [b] = [11]
    ...
    SECTION("Diagonal-only (decoupled) source terms " + execName)
    {
        //   [4  0] [a]   [8]     →  a = 2, b = 3
        //   [0  3] [b] = [9]
        auto sp = std::make_shared<SparsityPattern<localIdx>>(
    ...
    {
        // 3-cell diagonal sparsity: each cell only connects to itself
        auto sp = std::make_shared<SparsityPattern<localIdx>>(
