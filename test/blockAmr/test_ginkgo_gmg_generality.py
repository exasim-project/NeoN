...
# COLOUR sweeps (gmgPrecond.hpp rbgsSmooth), and the coarse hierarchy truncates
...
def _make_solver_or_skip(coeffs, geom, executor, **kwargs):
    ...
    try:
        return blockamr.FaceCoeffSolver(*coeffs, geom, executor=gko_executor(executor), **kwargs)
...
# --- our own Krylov bottom (gmg_bottom_solver) -----------------------------
#
# The tests above establish the gap on MLMG's side: a smoothing-only bottom costs
# it ~2x the cycles. gmg_bottom_solver puts the same Krylov bottom on OUR V-cycle,
# generated by Ginkgo on the coarsest level (linearAlgebra/gmg/gmgBottom.hpp), so these are
# the tests of what that buys and of what it must not break.
#
# Why a smoothing-only bottom cannot simply be given more sweeps: a consistent
# polynomial smoother has p(0) = 1, so the coarse grid's constant mode is untouched
# by every sweep. That is a floor on the V-cycle's contraction, not a tuning issue.

# Stationary V-cycle with the bottom solved by Ginkgo's CG instead of by sweeps.
# gmg_coarsest_sweeps is ignored on this path, so it is left out rather than set
# to a value that does nothing.
KRYLOV_BOTTOM = {
    "solver": "gmg",
    "max_iter": STATIONARY_BUDGET,
    "rtol": RTOL,
    "gmg_min_bottom": 2,
    "gmg_bottom_solver": "cg",
}


@pytest.mark.parametrize("executor", ["reference", "cuda"])
def test_krylov_bottom_matches_smoother_bottom(blockamr_session, executor):
    """The Ginkgo bottom solve is a drop-in: same operator, same answer.

    Guards the new path's correctness rather than its speed -- GmgBottomOp rebuilds
    the coarse stencil, fills its own ghosts and packs to a Ginkgo vector, and any
    of those going wrong would change the answer, not just the cycle count.
    """
    shape = (16, 16, 16)
    values = _random_values(shape)
    stats_s, sol_s, _, referee = _run(shape, executor, STATIONARY, values=values)
    stats_k, sol_k, _, _ = _run(shape, executor, KRYLOV_BOTTOM, values=values)
    assert stats_s["converged"] is True, "smoother bottom did not converge"
    assert stats_k["converged"] is True, "krylov bottom did not converge"
    assert _rel_residual(sol_k, values, referee) < AGREE
    diff = float(np.max(np.abs(sol_s - sol_k)))
    assert diff < 1e-8, f"krylov and smoother bottoms disagree by {diff}"


@pytest.mark.parametrize("executor", ["reference", "cuda"])
def test_krylov_bottom_survives_decomposition(blockamr_session, executor):
    """The multi-box case the smoothing-only bottom cannot do.

    test_native_gmg_is_decomposition_independent is xfail because coarsening stops
    as soon as a BOX stops being coarsenable, so many small boxes leave a bottom grid
    far too large for a fixed number of sweeps (measured 10 -> 66 cycles at N=16 for
    1 -> 64 boxes). That is precisely a bottom-solve failure, so a Krylov bottom
    should fix it WITHOUT the agglomeration the xfail message blames -- the bottom
    grid is still large, it is just actually solved now.

    The contrast is asserted in the same test, on the same decomposition, because
    "the Krylov bottom converges here" only means something next to "the smoothing
    bottom does not". Note there is nothing to prove SINGLE-box at this size: the
    file's STRONG_BOTTOM already spends 64 sweeps on a 2^3 bottom, which is enough,
    and both bottoms then converge in the V-cycle's own 10 cycles. The bottom only
    becomes the bottleneck once the decomposition makes the bottom grid large.
    """
    shape = (16, 16, 16)
    values = _random_values(shape)
    stats1, sol1, _, referee = _run(shape, executor, KRYLOV_BOTTOM, values=values)
    stats64, sol64, _, _ = _run(shape, executor, KRYLOV_BOTTOM, values=values, max_size=4)
    assert stats1["converged"] is True, "single box did not converge"
    assert stats64["converged"] is True, (
        f"64 boxes did not converge in {STATIONARY_BUDGET} cycles with a Krylov bottom"
    )
    assert _rel_residual(sol64, values, referee) < AGREE
    assert float(np.max(np.abs(sol1 - sol64))) < AGREE

    # The same run with the shipped smoothing bottom, which is what
    # test_native_gmg_is_decomposition_independent records as failing.
    stats_s, _, _, _ = _run(shape, executor, STATIONARY, values=values, max_size=4)
    assert stats_s["converged"] is False, (
        "the smoothing bottom converged at 64 boxes -- if the underlying gap has been "
        "closed, this contrast and the xfail on "
        "test_native_gmg_is_decomposition_independent should both go"
    )


@pytest.mark.parametrize("executor", ["reference", "cuda"])
def test_stationary_solve_reports_its_contraction(blockamr_session, executor):
    """A grinding V-cycle says so, instead of only reporting that it ran out of cycles.

    The 64-box case is the one where the smoothing bottom is genuinely the problem, so
    it is the case that has to name the bottom solve. The Krylov run on the SAME
    decomposition is the control: a healthy cycle must not emit a diagnostic, or the
    signal is noise. Measured contractions are ~0.59 and ~0.06 respectively, so the
    bounds below are placed either side of the emit threshold (0.464) with margin
    rather than fitted to those numbers.
    """
    shape = (16, 16, 16)
    values = _random_values(shape)

    bad, _, _, _ = _run(shape, executor, STATIONARY, values=values, max_size=4)
    assert bad["converged"] is False
    assert bad["contraction"] > 0.5, (
        f"expected a slow contraction on the case that does not converge, got {bad['contraction']}"
    )
    assert "gmg_bottom_solver" in bad["diagnostic"], bad["diagnostic"]

    good, _, _, _ = _run(shape, executor, KRYLOV_BOTTOM, values=values, max_size=4)
    assert good["converged"] is True
    assert good["contraction"] < 0.4
    assert good["diagnostic"] == "", f"healthy cycle emitted: {good['diagnostic']}"


def _bottom_solver_kwargs(**over):
    """FaceCoeffSolver kwargs for the guard tests, which never reach a solve."""
    kw = {"solver": "gmg", "max_iter": 1, "rtol": RTOL, "gmg_min_bottom": 2}
    kw.update(over)
    return kw


# Constructed directly rather than through _make_solver_or_skip: that helper turns a
# RuntimeError into a skip, which would swallow exactly the errors under test.
@pytest.mark.parametrize(
    "over, expect",
    [
        ({"gmg_bottom_solver": "nope"}, "unknown gmg_bottom_solver"),
        ({"symmetric": False, "gmg_bottom_solver": "cg"}, "needs a symmetric operator"),
        ({"symmetric": False, "gmg_bottom_solver": "fcg"}, "needs a symmetric operator"),
        ({"symmetric": False, "gmg_omega": 1.1}, "assumes a symmetric operator"),
        ({"symmetric": False, "gmg_smoother": "chebyshev"}, "assumes a symmetric operator"),
    ],
    ids=["unknown", "cg-asym", "fcg-asym", "omega-asym", "chebyshev-asym"],
)
def test_asymmetric_and_unknown_bottom_are_refused(blockamr_session, over, expect):
    """Configurations that would silently produce a wrong answer are refused.

    None of these fail loudly at runtime: a CG bottom on an asymmetric operator, an
    over-relaxed non-self-adjoint sweep, or a Chebyshev polynomial built on a real
    interval for a complex spectrum all converge to something, or stall, and the
    caller sees only a worse iteration count. So they raise at construction instead.
    """
    if not hasattr(blockamr, "FaceCoeffSolver"):
        pytest.skip("blockamr.FaceCoeffSolver binding not available")
    shape = (8, 8, 8)
    ...
    with pytest.raises(RuntimeError, match=expect):
        blockamr.FaceCoeffSolver(
            ...
            **_bottom_solver_kwargs(**over),
        )


# ---------------------------------------------------------------------------
# 3b. asymmetric operators
# ---------------------------------------------------------------------------
# Every solve above is of a SYMMETRIC operator: _coeffs hands the same face MultiFab
# to ux and lx, so aE(i) == aW(i+1) by construction. That left symmetric=False and
# the non-symmetric bottom solvers covered only by their refusal branches — nothing
# proved the asymmetric path computes the right answer. These tests supply genuinely
# different upper and lower faces.
#
# The operator is first-order upwind convection plus diffusion, i.e. the
# convection-dominated case the flag exists for. Upwinding needs nothing special
# from the kernel's diagonal rule: for velocity U > 0 along x it puts -U/dx on the
# WEST face only, and diag = alpha - sum(upper + lower) then picks up the matching
# +U/dx by itself. It also stays an M-matrix, so the RB-GS smoother is still a
# smoother — an asymmetry that broke diagonal dominance would be testing two things
# at once.


def _upwind_coeffs(geom, ba, dm, shape, vel, b_val=1.0, a_val=1.0):
    """(solver args, referee) for a*u - div(b grad u) + U du/dx, upwinded, U > 0.

    The x pair is the asymmetric one: ux is pure diffusion, lx carries the whole
    convective term. y and z get two SEPARATE MultiFabs holding equal values, so the
    solver cannot rely on upper and lower being the same object.
    """
    dx = geom.cell_size()
    upper, lower = [], []
    for d in range(3):
        ns = list(shape)
        ns[d] += 1
        f = np.full(ns, -b_val / dx[d] ** 2)
        upper.append(f)
        lower.append(f - vel / dx[0] if d == 0 else f.copy())
    mfs = [_face_mf(ba, dm, d, arr) for d in range(3) for arr in (upper[d], lower[d])]
    return (_const_cell(ba, dm, a_val), *mfs), (a_val, upper, lower)


def _rel_residual_asym(sol, rhs, referee):
    """||rhs - A sol||_2 / ||rhs||_2 for an operator with distinct upper/lower faces.

    Same stencil as _rel_residual, which cannot be reused because it derives both
    coefficients from a single face array — the very assumption under test here.
    """
    a_val, uf, lf = referee
    upper = [uf[0][1:, :, :], uf[1][:, 1:, :], uf[2][:, :, 1:]]
    lower = [lf[0][:-1, :, :], lf[1][:, :-1, :], lf[2][:, :, :-1]]
    diag = a_val - sum(upper[d] + lower[d] for d in range(3))
    au = diag * sol
    for d in range(3):
        au = au + upper[d] * np.roll(sol, -1, axis=d) + lower[d] * np.roll(sol, 1, axis=d)
...
def _run_upwind(shape, executor, cfg, values, vel, max_size=None, **solver_kw):
    """As _run, on the upwind operator. Device objects stay local for the same reason."""
    geom, ba, dm = _make_mesh(shape, max_size=max_size)
    coeffs, referee = _upwind_coeffs(geom, ba, dm, shape, vel)
    rhs = _cell_mf(ba, dm, values)
    solver = _make_solver_or_skip(coeffs, geom, executor, **cfg, **solver_kw)
    sol = _zero_sol(ba, dm)
    stats = dict(solver.solve(rhs, sol))
    return stats, _gather(sol, shape), referee


# Cell Peclet number U*dx/b at shape=(16,16,16): 32 * (1/16) / 1 = 2, so the west
# face coefficient is 3x the east one — convection-dominated, not a perturbation.
UPWIND_VEL = 32.0

# BiCGStab rather than CG on the bottom, because the coarse operator inherits the
# asymmetry. gmg_omega has to be spelled out: its default is 1.1, and symmetric=False
# refuses over-relaxation (see the refusal test above), so an asymmetric solve does
# not fall out of the defaults — it is a deliberate configuration.
ASYM = {
    "symmetric": False,
    "gmg_omega": 1.0,
    "gmg_min_bottom": 2,
    "gmg_bottom_solver": "bicgstab",
}

ASYM_STATIONARY = {"solver": "gmg", "max_iter": STATIONARY_BUDGET, "rtol": RTOL, **ASYM}

# Gcr rather than Cg as the outer solver: Cg and Fcg are refused on symmetric=False.
ASYM_PRECONDITIONED = {"solver": "gcr", "precond": "gmg", "max_iter": 400, "rtol": RTOL, **ASYM}


@pytest.mark.parametrize("multibox", MULTIBOX, ids=MULTIBOX_IDS)
@pytest.mark.parametrize("executor", ["reference", "cuda"])
def test_upwind_convection_is_solved(blockamr_session, executor, multibox):
    """The asymmetric operator is solved to the same tolerance as the symmetric one.

    Gcr-preconditioned rather than stationary because the stationary V-cycle does not
    reach RTOL here — see test_stationary_vcycle_on_upwind_convection below, which
    records that separately. This test is about the asymmetric path computing the
    right answer, so it uses the configuration in which it does.

    The last assertion is the non-vacuity check: the SYMMETRIC solve of the same rhs
    must NOT satisfy this referee. Without it the test would still pass if the
    convective term were dropped somewhere between the MultiFab and the stencil,
    because a symmetric solution is a perfectly good solution of a symmetric problem.
    """
    shape = (16, 16, 16)
    values = _random_values(shape)
    stats, sol, referee = _run_upwind(
        shape, executor, ASYM_PRECONDITIONED, values, UPWIND_VEL, max_size=_split(shape, multibox)
    )
    assert stats["converged"] is True, f"upwind solve did not converge: {stats}"
    assert _rel_residual_asym(sol, values, referee) < AGREE

    _, sym_sol, _, _ = _run(shape, executor, STATIONARY, values=values)
    assert _rel_residual_asym(sym_sol, values, referee) > 0.1, (
        "the symmetric solution satisfies the asymmetric operator — the convective "
        "term is not reaching the stencil and this test proves nothing"
    )


@pytest.mark.parametrize("executor", ["reference", "cuda"])
@pytest.mark.xfail(
    reason="the stationary V-cycle loses its fast contraction on convection-dominated "
    "asymmetry: measured a contraction of ~0.85 per cycle (residual 63.8 -> 2.0e-2 in "
    "30 cycles) at cell Peclet 2, N=16, versus 0.070 on the symmetric operator, because "
    "volume-average restriction and piecewise-constant prolongation are not aligned "
    "with the flow direction. Closing it needs matrix-dependent transfer operators or "
    "a downwind-ordered smoother, neither of which we have. Gcr with the same V-cycle "
    "as preconditioner converges — that is what test_upwind_convection_is_solved uses.",
    strict=False,
)
def test_stationary_vcycle_on_upwind_convection(blockamr_session, executor):
    """The stationary V-cycle should solve the upwind operator too. It grinds instead."""
    shape = (16, 16, 16)
    values = _random_values(shape)
    stats, sol, referee = _run_upwind(shape, executor, ASYM_STATIONARY, values, UPWIND_VEL)
    assert stats["converged"] is True
    assert _rel_residual_asym(sol, values, referee) < AGREE


@pytest.mark.parametrize("outer", ["gcr", "fcg"])
@pytest.mark.parametrize("executor", ["reference", "cuda"])
def test_flexible_outer_solver_tolerates_an_adaptive_bottom(blockamr_session, executor, outer):
    """The case gcr/fcg were added for: a bottom solve that is not a fixed operator.

    A Krylov bottom stopped on a LOOSE relative tolerance takes a different number of
    iterations per apply, so the V-cycle is a different linear operator every time —
    which is exactly the assumption Cg and Bicgstab make and these two do not. The
    operator here is symmetric, so Fcg is admissible alongside Gcr.
    """
    shape = (16, 16, 16)
    values = _random_values(shape)
    cfg = {
        ...
        "gmg_min_bottom": 2,
        "gmg_bottom_solver": "cg",
        "gmg_bottom_rtol": 1e-2,
    }
    stats, sol, _, referee = _run(shape, executor, cfg, values=values)
    assert stats["converged"] is True, f"{outer} did not converge: {stats}"
    assert _rel_residual(sol, values, referee) < AGREE
...
def test_multirank_residual_norm_is_global(blockamr_session, executor, norm):
    ...
    linf). Measured at 2 ranks before gmgKernels.hpp gained reduceResidNorms: the
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # across ranks (linOpBase.hpp makeGlobalVec, persistent.cpp).
        pytest.param(PRECONDITIONED, id="cg+gmg"),
        # Same treatment applied to the V-cycle's bottom solve, which is a Ginkgo
        # Krylov solver like any other (gmgPrecond.hpp). Without it the native
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Cg<float> needed the distributed views of its own (mixedPrecision.hpp) --
