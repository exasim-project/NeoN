...
* validation errors (bc vs geometry periodicity, an unknown bc value);
...
def test_bc_validation_errors(blockamr_session):
    """bc entries must match the geometry's periodicity, in both directions.

    The check lives in parseBc, which every path that takes a `bc` calls.
    """
...
# ---------------------------------------------------------------------------
# 3. The Helmholtz (alpha=1) manufactured mixes
#
# Section 1 solves the pure Poisson operator under all-Dirichlet and the
# Helmholtz one under all-Neumann / mixed. The rows below run the same three u
# under alpha=1 throughout, so the all-Dirichlet case is exercised on a
# NONSINGULAR operator too rather than only on the singular-adjacent one.
# ---------------------------------------------------------------------------


# All three are eigenfunctions of -lap with eigenvalue 3 pi^2, so one rhs formula
# covers them; alpha=1 keeps every mix nonsingular.
def _u_helmholtz_dirichlet(x, y, z):
    pi = math.pi
    return np.sin(pi * x) * np.sin(pi * y) * np.sin(pi * z)


def _u_helmholtz_neumann(x, y, z):
    pi = math.pi
    return np.cos(pi * x) * np.cos(pi * y) * np.cos(pi * z)


def _u_helmholtz_mixed(x, y, z):
    pi = math.pi
    return np.sin(pi * x) * np.cos(pi * y) * np.cos(pi * z)
...
@pytest.mark.parametrize(
    "kind, bc, u_fn",
    [
        ("dirichlet", ["dirichlet"] * 6, _u_helmholtz_dirichlet),
        ("neumann", ["neumann"] * 6, _u_helmholtz_neumann),
        (
            "mixed",
...
def test_helmholtz_manufactured_second_order(blockamr_session, executor, kind, bc, u_fn):
    """u - lap u = f: 2nd order under each bc mix, same thresholds as section 1."""
