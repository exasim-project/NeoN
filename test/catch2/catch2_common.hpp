 ...
 * an "ambiguous partial specializations" error under NVCC's stricter resolver
 * (g++ tolerates the ambiguity; nvcc -arch=sm_80 does not). Disabling the
 * range trait lets the NeoN-specific specialization win unambiguously and
 * ensures device-resident vectors are copied to host before stringification.
 */
template<typename T>
